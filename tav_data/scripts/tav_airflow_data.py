...
for f in [str(f) for f in [6,7,2,1]]:
  ...
  floor_path = '../csv_output/Floor/' + 'Floor%s_airflow' %(str(f).zfill(2))  \
  ...
  floor_ratio_path = '../csv_output/Floor/' + 'Floor%s_ratios' %(str(f).zfill(2))  \
      + '%s-%s'%(startF.strftime("%Y%m%d"), endF.strftime("%Y%m%d"))\
      + '.csv' 
